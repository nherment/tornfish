...
var logger          = require('../util/Logger.js').get('database')
...
Provider = function(name, host, port, user, password, callback) {
  ...
  this.loadCollection = function(name, indexes, callback) {
    ...
    logger.debug('Loading collection ['+name+']')
    ...
    self.db.collection(name, function(err, collection) {
      ...
      logger.debug('Collection ['+name+'] loaded')
...
var dbConf = {
  host      : process.env.TORNFISH_DB_HOST || 'localhost',
  port      : process.env.TORNFISH_DB_PORT || '27017',
  name      : process.env.TORNFISH_DB_NAME || 'tornfish',
  user      : process.env.TORNFISH_DB_USER || undefined,
  password  : process.env.TORNFISH_DB_PWD  || undefined
}
