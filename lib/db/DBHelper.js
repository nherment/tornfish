...
function Helper(collectionName, keyName, config) {
  ...
  function load(callback) {
    if(dbConnection) {
      return loadCollection(callback)
    }
    DB(function(db) {
      dbConnection = db
...
DBHelper.Server = new Helper("servers", "name", [ { index: {name: 1}, options: {unique: true} } ])
...
DBHelper.Config = new Helper("configs", "name", [ { index: {name: 1}, options: {unique: true} } ])
DBHelper.Image  = new Helper("images",  "name", [ { index: {name: 1}, options: {unique: true} } ])
